/**
 * This file is copied from an assignment handout for the course TDT4200 on NTNU
 */
#include <stdio.h>
#include <stdlib.h>
...
void write_bmp(unsigned char* data, int width, int height){
    ...
    header.file_size = (uint32_t)(width*height+54 + 2);
    ...
    dib_header.data_size = (uint32_t)(width*height);
    dib_header.hres = 0;
    dib_header.vres = 0;
    ...
    printf("%p\n", data);
    fwrite((void*)data, 1, (uint32_t)(width*height*3), fp);
...
unsigned char* read_bmp(char* filename){
    ...
    unsigned char* data = (unsigned char*)malloc(sizeof(unsigned char)*(unsigned long)(height*width));

    fseek(fp, offset, SEEK_SET);
    //We just ignore the padding :)
    fread(data, sizeof(unsigned char), (size_t)(height*width), fp);
